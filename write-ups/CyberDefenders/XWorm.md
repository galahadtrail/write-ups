...
Для ответа на следующий вопрос о свойствах процесса, которое предоставит большие права для предотвращения обнаружения/сброса нам опять поможет **JoeSandbox**. В описании процессов мы сможем среди прочего найти строку *RtlSetProcessIsCritical* Именно она помечает процесс как критический. (**№14 - RtlSetProcessIsCritical**)

Далее нас спрашивают, какую функцию WindowsAPI использует образец для сбора нажатий клавиатуры (перехвата сигналов нажатий). Такие функции для взаимодействия с клавиатурой могут быть импортированы только из **user32.dll** Ищем в поиске **dnSpy** эту библиотеку и находим ответ - используется **SetWindowsHookEx** (**№15 -** **SetWindowsHookEx**)

![API для перехвата клавиш](Attachments/keylog-Xworm.png)

Последний вопрос касается класса, который представляет собой образец с таким функционалом. Вредонос, который перехватывает (считывает, фиксирует) нажатия клавиатуры в общепринятой терминологии называется keylogger (кейлоггер). Это и будет ответом (**№16 - Keylogger**).

Практически всю работу можно было решить с помощью отладки в **dnSpy** (*или аналогичной ей программе*). Однако без запуска вредоноса, строго *на статическом анализе*, приходится использовать сторонние сервисы, т.к. некоторые параметры зашифрованы (закодированы) и раскрыть их содержание можно только в виде переменных в процессе запуска.